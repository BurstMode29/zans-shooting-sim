...
export default function ReflexesSessionDetail({ sessionData }) {
    return (
        <View>
            <CardGraph
                title={"Delay"}
                description={"Seconds delay between prompt and shot."}
                labels={sessionData.shots.map((shot, index) => { return `${index + 1}` })}
                datasets={[{
                    data: sessionData.shots.map((shot, index) => {
                        if ("delaySeconds" in shot) {
                            return shot.delaySeconds
                        }
                        return 0;
                    })
                }]}>
